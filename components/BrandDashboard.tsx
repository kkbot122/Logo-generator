      ...
      <div className="flex justify-between items-center mb-12">
        ...
        <div className="flex gap-4">
          ...
          <PDFDownloadLink
            ...
            fileName={`${brand.brandName
              .replace(/\s+/g, "-")
              .toLowerCase()}-brand-kit.pdf`}
          ...
          >
            {({ loading }) =>
              loading ? "Preparing PDF..." : "Download Brand Kit"
            }
          </PDFDownloadLink>
        </div>
      </div>
          ...
          <div className="bg-white border border-black/10 p-8 rounded-sm shadow-sm">
            ...
            <div className="grid grid-cols-5 h-24 w-full">
              {brand.colors.palette.map((hex: string) => (
                <div
                  key={hex}
                  onClick={() => copyToClipboard(hex)} // <--- Add Click Handler
                  className="group relative h-full w-full first:rounded-l-md last:rounded-r-md cursor-pointer hover:scale-105 transition-transform z-0 hover:z-10 shadow-none hover:shadow-lg"
                  style={{ backgroundColor: hex }}
                >
                  {/* Tooltip */}
                  <div className="opacity-0 group-hover:opacity-100 absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-black text-white text-[10px] px-2 py-1 rounded whitespace-nowrap pointer-events-none">
                    Click to Copy {hex}
                  </div>
                </div>
              ))}
          ...
          <div className="bg-white border border-black/10 p-8 rounded-sm shadow-sm">
            ...
            <div className="border-l-4 border-black pl-6 py-2">
              ...
              <p className="text-neutral-500 text-sm leading-relaxed">
                A {brand.fonts.category} selection ideal for modern brands. Use
                this typeface for headlines and primary messaging to communicate
                confidence.
              </p>
            </div>
          </div>
